...
module.exports = function (grunt) {
	...
	grunt.initConfig({
		...
		compress: {
			...
			finalWindowsApp: {
				options: {
					archive: '<%= config.dist %>/adventure-editor.zip'
				},
				files: [{
					expand: true,
					cwd: '<%= config.tmp %>',
		...
		rename: {
			app: {
				files: [{
					...
					dest: '<%= config.dist %>/adventure-editor.app'
				}]
			},
			zipToApp: {
				files: [{
	...
	grunt.registerTask('chmod', 'Add lost Permissions.', function () {
		var fs = require('fs');
		fs.chmodSync('dist/adventure-editor.app/Contents/Frameworks/node-webkit Helper EH.app/Contents/MacOS/node-webkit Helper EH', '555');
		fs.chmodSync('dist/adventure-editor.app/Contents/Frameworks/node-webkit Helper NP.app/Contents/MacOS/node-webkit Helper NP', '555');
		fs.chmodSync('dist/adventure-editor.app/Contents/Frameworks/node-webkit Helper.app/Contents/MacOS/node-webkit Helper', '555');
		fs.chmodSync('dist/adventure-editor.app/Contents/MacOS/node-webkit', '555');
	});
	...
	grunt.registerTask('createWindowsApp', 'Create windows distribution.', function () {
		...
		var childProcess = require('child_process');
		var exec = childProcess.exec;
		exec('copy /b tmp\\nw.exe+tmp\\app.nw tmp\\adventure-editor.exe && del tmp\\app.nw tmp\\nw.exe', function (error, stdout, stderr) {
			var result = true;
			if (stdout) {
				grunt.log.write(stdout);
			}
	...
	grunt.registerTask('dist-linux', ['jshint', 'clean:dist', 'copy:appLinux', 'createLinuxApp']);
	grunt.registerTask('dist-win', ['jshint', 'clean:dist', 'copy:copyWinToTmp', 'compress:appToTmp', 'rename:zipToApp', 'createWindowsApp', 'compress:finalWindowsApp']);

	grunt.registerTask('dist-mac', ['jshint', 'clean:dist', 'copy:webkit', 'copy:appMacos', 'rename:app', 'chmod']);
	grunt.registerTask('check', ['jshint']);

	grunt.registerTask('dmg', 'Create dmg from previously created app folder in dist.', function () {
		var done = this.async();
		var createDmgCommand = 'resources/mac/package.sh "adventure-editor"';
